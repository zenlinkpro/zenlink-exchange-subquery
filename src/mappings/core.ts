...
export function handleTransfer(event: Transfer): void {
  ...
  if (event.params.to.toHexString() == ADDRESS_ZERO.toHexString() && event.params.value.equals(BigInt.fromI32(1000))) {
    return
  }
  ...
  // burn
  if (event.params.to.toHexString() == ADDRESS_ZERO.toHexString() && event.params.from.toHexString() == pair.id) {
  ...
  if (from.toHexString() != ADDRESS_ZERO.toHexString() && from.toHexString() != pair.id) {
  ...
  if (event.params.to.toHexString() != ADDRESS_ZERO.toHexString() && to.toHexString() != pair.id) {
...
export function handleSync(
  ...
  let uniswap = Factory.load(FACTORY_ADDRESS.toHexString())
