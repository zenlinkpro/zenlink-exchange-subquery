...
import { numberToBigint, ONE_BI, ZERO_BI } from './helpers';
...
export async function updatePairDayData(event: MoonbeamEvent): Promise<PairDayData> {
  ...
  if (!pairDayData) {
    ...
    pairDayData.date = numberToBigint(dayStartTimestamp)
...
export async function updatePairHourData(event: MoonbeamEvent): Promise<PairHourData> {
  const timestamp = event.blockTimestamp.getTime()
  const hourIndex = BigNumber.from(timestamp).div(3600).toNumber() // get unique hour within unix history
  ...
  if (!pairHourData) {
    ...
    pairHourData.hourStartUnix = numberToBigint(hourStartUnix)
...
export async function updateTokenDayData(token: Token, event: MoonbeamEvent): Promise<TokenDayData> {
  ...
  if (!tokenDayData) {
    ...
    tokenDayData.date = numberToBigint(dayStartTimestamp)
